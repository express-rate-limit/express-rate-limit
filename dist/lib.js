...
const parseOptions = (passedOptions) => {
    ...
    store = new MemoryStore(windowMs), } = passedOptions;
    ...
    const options = {
        ...
        skip: () => false,
        handler: (_request, response, _optionsUsed) => response.status(options.statusCode).send(options.message),
        onLimitReached: () => { },
        ...passedOptions,
    };
    // Ensure that the store passed implements the {@link Store} interface
    if (typeof options.store.increment !== 'function' ||
        typeof options.store.resetKey !== 'function' ||
        (options.skipFailedRequests &&
            typeof options.store.decrement !== 'function')) {
        throw new Error('An invalid store was passed. Please ensure that the store is a class that implements the `Store` interface.');
    }
    // Throw an error if any deprecated options are passed
    const deprecatedOptions = ['global', 'delayMs', 'delayAfter'];
    for (const option of deprecatedOptions) {
        ...
        if (passedOptions[option]) {
            throw new Error(`The \`${option}\` option is deprecated and will likely be removed from the \`express-rate-limit\` package in the future.`);
        }
    }
...
const rateLimit = (passedOptions) => {
    ...
    const middleware = handleAsyncErrors(async (request, response, next) => {
        ...
        const { totalHits, resetTime } = await new Promise((resolve, reject) => {
            options.store.increment(key, (error, totalHits, resetTime) => {
                if (error)
                    reject(error);
