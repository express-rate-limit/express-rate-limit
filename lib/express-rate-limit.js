    ...
    options.store.incr(key, function(err, current, resetTime) {
      ...
      Promise.resolve(maxResult)
        ...
        .then(max => {
          ...
          if (options.headers) {
            options.setHeader("X-RateLimit-Limit", max);
            options.setHeader("X-RateLimit-Remaining", req.rateLimit.remaining);
            if (resetTime instanceof Date) {
              // if we have a resetTime, also provide the current date to help avoid issues with incorrect clocks
              options.setHeader("Date", new Date().toGMTString());
              options.setHeader(
                "X-RateLimit-Reset",
          ...
          if (max && current > max) {
            if (options.headers) {
              options.setHeader("Retry-After", Math.ceil(options.windowMs / 1000));
            }
