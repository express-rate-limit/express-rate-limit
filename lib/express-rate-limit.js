...
function RateLimit(options) {
    ...
    function rateLimit(req, res, next) {
        ...
        options.store.incr(key, function(err, current) {
            ...
            if (options.max && current.count > options.max) {
              if(current.violation === undefined) {
                options.onViolation(req, res, options);
            ...
            if (options.delayAfter && options.delayMs && current.count > options.delayAfter) {
              var delay = (current.count - options.delayAfter) * options.delayMs;
