...
> Thanks to [Mintlify](https://www.mintlify.com), for generously hosting this
> documentation.
...
clients to be stored in an external database, or in-memory.

<Warning>
	The default `MemoryStore` stores the hit counts for clients in memory, and
...
an [external data store](/docs/reference/configuration#store).
...
[the contributing guide](/docs/guides/contributing). Then you can pick up any
