...
- IPv6 addresses are now grouped with a /56 subnet by default and rate-limited
  together. For example, the following two IP addresses will now be considered
  to be the same user in the default configuration (both would be normalized to
