...
- `ipKeyGenerator(ip, ipv6Subnet)` helper method to apply the desired subnet to IPv6 addresses (returns IPv4 unchanged).
- `ipv6Subnet` configuration option used by the default `keyGenerator`, defaults to `56`
- `ipv6Subnet` validation check on above configuration option's value (allowed range is 32-64)
- `keyGeneratorIpFallback` validation check on custom `keyGenerator`s to ensure they're using `ipKeyGenerator` if they reference `req.ip` or `request.ip`
