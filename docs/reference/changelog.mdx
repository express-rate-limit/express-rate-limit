...
Note: a semver-patch version increment was chosen for this release when it could
have been considered a minor or arguably even a major version change because we
believe that the core issue is a bug in the library, and the changes are in
service of fixing that bug. Additionally, there is evidence of a similar
vulnerability being exploited in the wild, and precident for similar fixes being
provide in patch releases for other software, e.g. Nextcloud server:
https://www.cve.org/CVERecord?id=CVE-2021-22915

### Fixed

- Fixed a vulnerability where IPv6 users could bypass rate limiting by
  iterating through multiple IP addresses in their ISP-assigned subnet.
