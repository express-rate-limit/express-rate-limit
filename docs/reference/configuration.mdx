...
Constants for `DAY`, `HOUR`, `MINUTE`, and `SECOND` are provided to make the
`windowMs` configuration a little easier to work with.

For example, to set a 3 hour window:
...
Or, for a 1 minute and 30 second window:

```ts
import { rateLimit, MINUTE, SECOND } from 'express-rate-limit'

const limiter = rateLimit({
	windowMs: 1 * MINUTE + 30 * SECOND,
	// ...
})
```
...
> Added in `8.0.0` (/56 default), `7.6.0` (/64 default).
...
The default value is 56 on 8.x and newer, corresponding to a /56 subnet. (The
feature was backported with a more relaxed default of 64 on earlier versions.)
...
When enabled, a set of validation checks are run early on to detect common
misconfigurations with proxies, etc. Prints an error to the console if any issue
is detected.
...
Debug logging is controlled via the `DEBUG` environment property rather than a
configuration option. See [Debugging Express Rate Limit](/guides/debugging) for
additional details.
