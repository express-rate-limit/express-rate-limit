...
addresses, whereas they usually provide only a single IPv4 address per customer.
A malicious user could iterate through their range of IPv6 addresses and bypass
simple IP-based rate limiting. This setting counteracts that by allowing the
rate-limiter to block an entire range of IPv6 addresses at once.

The default value is 56, corresponding to a /56 subnet. This means that in a
128-bit IPv6 address, only the first 56 bits are considered for rate limiting.
The remaining 72 bits are assumed to be under the end user's control and
ignored.

Technically the valid range is 1-128, but typically ISPs assign subnets in the
range of 48-64, with 64 being the most common. A validation check will warn for
values outside the 32-64 range, (but can be disabled).

56 is a moderately aggressive default. If users are being incorrectly blocked,
try 60 or 64. Alternatively, if you're seeing evidence of abuse, try 52 or 48.
...
apply different subnets to different different users.
