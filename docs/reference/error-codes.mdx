...
`keyGenerator` function does respect the value of the `ipv6Subnet` option - only
the default `keyGenerator` makes use of it.

To resolve this error, use only one of the two options in your configuration. If
your custom `keyGenerator` function falls back to using the IP address as a key,
use remember to wrap it with the [`ipKeyGenerator`](./helpers#ipkeygenerator)
method like [this](#err-erl-key-gen-ipv6).
