...
Once node and npm are installed, run `npm install -g pnpm` to install [pnpm](https://pnpm.io/).
...
If there are errors, try deleting the `node_modules` folder first, then re-run the install command.
